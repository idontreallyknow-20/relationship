...
export function sunGain(state: GameState, multiplier = 1): number {
  const past = overshoot(state.seaHearts, seaRequirement(state.seas));
  if (past <= 0) return 0;
  const waterBonus = 1 + Math.min(3, state.newWaters / 10);
  const shelfBonus = 1 + Math.min(1, (state.sealed?.length ?? 0) / 100);
  return Math.floor(past * 3 * waterBonus * shelfBonus * multiplier);
}
...
const RESET_TREE_SHAPE: Record<string, string[]> = {
  ...
  // and the last two rungs need both.
  d_depth: ["d_all"],
  d_depth_1: ["d_depth"],
  d_depth_2: ["d_depth_1"],
  d_depth_3: ["d_depth_2"],
...
export const SUN_UPGRADES: ResetUpgradeDef[] = shaped(SUN_RAW);

export const RESET_UPGRADE_BY_ID: Record<string, ResetUpgradeDef> = Object.fromEntries(
  [...MOON_UPGRADES, ...STAR_UPGRADES, ...SUN_UPGRADES].map((u) => [u.id, u]),
);
